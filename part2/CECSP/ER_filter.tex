...
\begin{ex}
  ...
  \begin{tabular}{|P{1cm}P{1cm}P{1cm}P{1cm}P{1cm}P{1cm}P{2cm}|}
    ...
    \hline
    i & 2 & 8 & 21 & 2 & 5 & b+3\\
    j & 0 & 8 & 22 & 2 & 5 & \frac{1}{2}*b+\frac{1}{2}\\
    \hline
  \end{tabular}
\end{center}
...
\begin{figure}[!htb]
  ...
  \subcaptionbox{Activité $i$}[0.45\linewidth]{
    \begin{tikzpicture}
      ...
      \draw[->] (-1,0) -- (-1,6);
      \draw[dashed] (10, 2) -- (0,2) -- (-0.1,2) node[left] {$\bmin$};
      \draw[dashed] (10, 5) -- (0,5) -- (-0.1,5) node[left] {$\bmax$};
...
utilisée, i.e. $f_i(b)=3b+1 \text{ si } b \in ]3,4]$.
...
l'algorithme de balayage de Bentley-Ottmann~\cite{sweep}. 

L'idée principale sur laquelle repose l'algorithme de balayage est que
deux segments ne peuvent s'intersecter s'ils ne leurs projections sur
l'axe des ordonnées et sur l'axe des abscisses ne se chevauchent
pas. Une ligne horizontale fictive est utilisée pour balayer l'axe des
...
\begin{theo}
  ...
  par la table~\ref{tab:interCheck_CECSP} avec:
  \begin{center}
    \begin{tabular}{lcr}
...
$\Delta(t_1)=\frac{W_i-f_i(\bmin)\LE
+t_1f_i(\bmax)}{f_i(\bmax)-f_i(\bmin)}=\smax$ et
$\Gamma(t_1)=\frac{W_i-t_1(f_i(\bmin)-f_i(\bmax))+\ES
f_i(\bmax)}{f_i(\bmin)}=\emin$
...
La figure~\ref{fig:variation} représente ces variations.
   \begin{figure}[ht]
 ...
 \noindent
où $expr_{ip}=\frac{1}{a_{ip}}\left(W_i-
f_i(\bmax)(\LE -t_2+t_1-\ES) - c_{ip}(t_2-t_1)\right)$.
...
\setstretch{1.35}
  ...
  \PourTous {$t_1 \in O_1$}{
    \PourTous {$i \in \A$}{
      ...
      condition~\eqref{eq:deriv2_CECSP} à l'aide de la
      table~\ref{tab:interCheck_t2}
...
\PourTous {$t_2 \in O_2$}{
  \PourTous {$i \in \A$}{
      ...
      condition~\eqref{eq:deriv1_CECSP} à l'aide de la
      table~\ref{tab:interCheck_t1}
...
t_1}$ et les points $t_2$ vérifiant $\frac{\delta^{-}\bbRS}{\delta
t_2} > \frac{\delta^{+}\bbRS}{\delta t_2}$.