...
figure~\ref{fig:ex_ER_CUSP}, nous représentons graphiquement é
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[xscale=0.35,yscale=0.5]
   ...
   \draw[->] (0,0) -- (9,0);
   \draw[thick] (3.5,0) node[below] {$\LE[1]$} -- (3.5, 8.5);
   \draw[thick] (0.5,0) node[below] {$\ES[1]$} -- (0.5, 8.5);
   \draw (0.5,0) rectangle (3.5,1) node[midway] {\small $LS$};
   \draw (0.5,3) rectangle (3.5,4) node[midway] {\small $RS$};
   \draw (0.5,6) rectangle (3.5,7) node[midway] {\small $CS$}; 
...
\begin{center}
  \begin{tabular}{|P{1cm}|P{4cm}P{4cm}P{3.5cm}P{1.5cm}|}
    ...
    \hline
    1 & 1*\max(0,p_i)=0 & 1*\max(0,p_i)=0 & 1*(6-3)=3 & 0 \\
