...
énergétique peuvent être trouvées dans~\cite{Alb}.
...
considérons l'ensemble $\O_1=\cup_{i \in \A} \{\langle i,\ES\rangle,\langle i,\LS\rangle\}$ des dates de
début possibles d'un intervalle et l'ensemble $\O_2=\cup_{i \in \A}
\{\langle i,\EE\rangle,\langle i,\LE\rangle \}$ correspondant aux dates de fin. De plus, nous
définissons l'ensemble $O_{t}=\cup_{i\in \A} \{\langle i,\ES+\LE -t\rangle\}$. Ceci
nous permet de décrire l'algorithme (cf.~\ref{algo:check_CUSP}) de
détection d'incohérence en $O(n^2)$ de Derrien~\cite{Alb}.
\begin{algorithm}[!htb]
\setstretch{1.35}
  \caption{Algorithme de détection d'incohérence énergétique pour le \CUSP.}
  \label{algo:check_CUSP}
  \PourTous {$\langle i,t_1\rangle \in \O_1$}{

    $pente=\sum_j \bb[j][t_1][t_1+1]$

    $SL_{ER}=0$, $t_2^{old}=t_1$

  \PourTous {$\langle j,t_2\rangle \in \O_2 \cup \O_{t_1}$ par ordre croissant}{
...
avant $\min\{t \in \H|t \in moi_i \}$ ($\H$ étant l'horizon de temps
du problème).
  \begin{figure}[!htb]
    \centering
    \begin{tikzpicture}
