...
\begin{center}
  \begin{tabular}{|P{1cm}|P{4cm}P{4cm}P{3.5cm}P{1.5cm}|}
    \hline
    act & \wbLS[i][3][6] & \wbRS[i][3][6] & \wbCS[i][3][6] & \wb[i][3][6]\\
    \hline
...
$t_2 \rightarrow \wb$, à $t_1$ fixé. Les variations de cette fonction
...
\begin{minipage}{\linewidth}
  \begin{minipage}{0.4\linewidth}
    \begin{itemize}
      ...
      \vspace{0.4cm}
    \item si $t_2 \le \LS$ alors $\wb=0$
      
      \vspace{0.4cm}
    \item si $\LS < t_2 < \LE$ alors $\wb= b_i (t_2 - \LS)$
      
      \vspace{0.4cm}
    \item si $\LE \le t_2 $ alors $\wb = p_i$
    \end{itemize}
  \end{minipage}
  \hfill
  \begin{minipage}{0.55\linewidth}
    ...
    \end{tikzpicture}
    \caption{{\'E}volution de $t_2 \rightarrow \wb$}
...
\setstretch{1.35}
  ...
  \PourTous {$(j,t_2) \in \O_2 \cup \O_{t_1}$ par ordre croissant}{
    ...
    $pente=pente + \wb[j][t_1][t_2-1] - 2\wb[j] +  \wb[j][t_1][t_2+1]$
...
disjonctif~\cite{Gay2015}. Dans ce paragraphe nous parlerons
brièvement du second et détaillerons le dernier qui sera adapté pour
le \CECSP. La raison pour laquelle nous avons choisi ce dernier sera
détaillée dans le chapitre~\ref{sec:PPC_CECSP}.
...
obligatoire. 
  \begin{figure}[!htb]
    \subcaptionbox{Ordonnancement au plus tôt}[0.3\linewidth]{
    \begin{tikzpicture}
      [xscale=0.25, yscale= 0.25,node distance=0.5cm]
      \node (sil) at (1,0) {} ;
      \node (eil) at (5,0) {} ;
      \node [below of=eil,node distance=0.63cm]  {$\EE$};
      \draw (sil.center) node[below=0.2cm] {$\ES$};
      ...
      \draw[densely dotted] (4.95,0) -- (4.95,3);
      \draw[<->] (0,0.1) -- (0,2) node[midway,left] {$b_i$};
      \draw[fill=white] (1,0) rectangle (5,2) node[midway] {$i$};
      \draw[line width=3pt] (1,0) -- (1,3);
...
\subcaptionbox{Ordonnancement au plus tard}[0.3\linewidth]{
    \begin{tikzpicture}
      ...
      \draw[densely dotted] (9.05,0) -- (9.05,3);
      \draw[<->] (14,0.1) -- (14,2) node[midway,right] {$b_i$};
      \draw[fill=white] (9,0) rectangle (13,2) node[midway] {$i$};
      \draw[line width=3pt] (13,0) -- (13,3);
...
\hfill
\subcaptionbox{Intervalle de superposition minimum}[0.3\linewidth]{ 
  \begin{tikzpicture}
    [xscale=0.25, yscale= 0.25,node distance=0.5cm]
    \node (sir) at (9,0) {} ;
    \node (eil) at (5,0) {} ;
    \node [below of=eil,node distance=0.63cm]  {$\EE$};
    \node[below of= sir,node distance=0.63cm] {$\LS$};
    \draw[<->] (4,2.2) -- (9,2.2) node[midway,label={}]
    {};
