\section{Amélioration des modèles}
...
permettant de le décrire. De plus, plusieurs ensemble d'inégalités valides
sont proposées pour contribuer à l'amélioration des performances des
modèles à événement.
 
\subsection{Amélioration du modèle indexé par le temps}
...
apporter à une activité une énergie $w$. En considérant toutes les
...
\begin{align}
&\left(1-\sum_{t<t_1}x_{it} - \sum_{t>t_2}y_{it}\right) \, \conv[W_i] +
...
\item l'activité commence avant $t_1$ et finit avant $t_2$. Ici,
  ...
  cas où l'activité n'est pas activée. 
...
ces inégalités sont calculées à la section~\ref{section:expe_TI} afin
...
\subsection{Motivations des modèles à événement}

Cette section montre l'intérêt de l'utilisation des modèles à
événement en comparant notamment le nombre de variables et de
contraintes de ces derniers. Pour le \CECSP, un exemple présentant une
instance ne comprenant que des données entières et ne possédant que
des solutions non-entières permet de 
démontrer l'importance de l'amélioration des modèles à événement.
...
Un autre argument en faveur des modèles à événement existe pour le
\CECSP. En effet, il peut arriver qu'une instance de ce problème ne
...
\begin{ex}
  ...
  La seule solution est décrite par la
  figure~\ref{figure_exemple_NE}.
  \begin{figure}[!htb]
    \centering
    \begin{tikzpicture}
      [xscale=2]
...
résolution efficaces pour le \RCPSP~et le \CECSP. 
...
d'inégalité est ensuite utilisé pour donner une description minimale
...
celle du modèle on/off. Pour cela, nous montrons que toute solution du 
modèle start/end, entière ou non, est solution du modèle on/off en
écrivant les variables $z_{ie}$ en fonction des variables $x_{ie}$ et
$y_{ie}$. Nous avons la relation suivante: 
\begin{equation}
\label{SEversOO}
...
Considérons le sous-modèle formée des
équations~\eqref{start_CECSP_OO}, \eqref{preem1_CECSP_OO} et
\eqref{preem2_CECSP_OO} et le sous-modèle formée des
...
Nous affirmons  que $z_a=(0.6,0,0.7,0)$ est bien une
solution du sous-modle on/off. 

Nous devonc maintenant montrer que le sous modèle start/end ne possède
pas de solution avec $z_a=(0.6,0,0.7,0)$. Pour cela, étudions les contraintes du
modèle qui n'implquent que l'activité $a$:
...
Or, la contrainte~(\ref{c18}) implque $y_{a1}=0.6+x_{a1}$
...
Donc, nous avosn bien montrer que le modèle start/end possède de
meilleures relaxations que le modèle on/off. Cependant, si nous
ajoutons un ensemble particulier d'inégalités à ce modèle, ceci n'est
plus le cas. Ces inégalités sont décrites dans le paragraphe suivant. 
...
L'ensemble d'inégalités, appelées inégalité de non-préemption, est
...
\end{equation}
où ${\cal F}=\{e_0,e_1,\dots,e_{2v}\}$ est un sous-ensemble de
cardinalité impaire de $\E^*=\Em$.  

Soit $ZP_i=\{z_i \in [0,1]^{\E^*}\ | \ z_i\text{ satisfait
\eqref{start_CECSP_OO} et \eqref{non_preem_ineg}}\}$ et soit
$ZQ_i=conv\{z_i \in \{0,1\}^{\E^*}\ | \ z_i\text{ satisfait
...
$ZQ_i$. Pour cela, commeçons par remarquer que les sommets de $ZQ_i$
...
rayons extrêmes du cône~\eqref{kis4} vérifiant~\ref{kis5} et les
inégalités de $ZP_i$.

Pour trouver les rayons extrêmes du cône~\eqref{kis4}, il suffit de
considérer les trois cas suivant: 
\begin{itemize}
\item $\mu_0=1$. Dans ce cas là, pour tout $e \in \E^*$, nous avons
  $\mu_e \le -1$. En effet, ceci est déduit de~\eqref{kis4} en
  considérant l'équation pour $u=v=e$. Alors~\eqref{kis5} implique:
\begin{align*}
&\sum_{e \in \E^*} \mu_e \overline{z}_{i,e}\le -1\\ 
\Rightarrow & \sum_{e \in \E^*} (-\mu_e)(-\overline{z}_{i,e})\le -1\\
\Rightarrow & \sum_{e \in \E^*} - \overline{z}_{i,e} \le -1 
\end{align*}
Donc nous avons bien l'inégalité suivante:
...
\item $\mu_0=0$. Alors, nous avons toujours un cône dont les rayons
  ...
  rayons extrêmes nous donnent les inégalités $z_{i,e} \le 0$ qui sont
  des contraintes de non-negativité valides pour $ZQ_i$. 
\item $\mu_0=-1$. Nous allons prouver que dans ce cas, il existe une
  ...
  inégalités de~\eqref{non_preem_ineg} est défini par:
  \[ \overline{\mu}_e=\left\{ 
      \begin{array}{ll}
  ...
  \end{align*}
  De plus, $L$ contient l'ensemble d'inégalité suivant, défini pour
  ...
  \end{align*} 
  On peut facilement vérifié que le système ci-dessus est formé de
  ...
  correspondante à un point extrême de $H$ équivaut à une inégalité
  de~\eqref{non_preem_ineg}. Dans un premier temps, remarquons que la
  matrice formée par les coefficients de la partie gauche
  de~\eqref{kis6} est totalement unimodulaire et donc que tout sommet de
...
\subsubsection{Autres inégalités}

Dans ce paragraphe nous décrivons plusieurs ensemble d'inégalités
...
$[{\cal D}_x^1,{\cal D}_y^1] \le [{\cal D}_x^2,{\cal D}_y^2]
\Leftrightarrow {\cal D}_x^1 < {\cal D}_x^2 \lor \left( {\cal
D}_x^1={\cal D}_x^2 \land {\cal D}_y^1\le {\cal D}_y^2\right)$

Alors, nous avons la propriété suivante:
\begin{equation} \label{sep_CECSP_OO} 
t_{e+1}-t_e \le |{\cal S}_e \cup
{\cal S}_{e+1}| \qquad \forall e \in {\cal E}\setminus\{2n\}
\end{equation}
...
\draw[->] (0,0) -- (23,0); \draw (1,0) node {[} node[above=0.2cm]
{$\ES[1]$}-- +(0:3cm)node {]} node[right=0.1cm,above=0.2cm]{$\LS[1]$};
\draw(7.05,0) node {[} node[above=0.2cm] {$\EE[1]$}-- +(0:3cm) node
{]} node[above=0.2cm] {$\LE[1]$}; \draw (3,0) node {[} node[below=0.3cm]
{$\ES[2]$}-- +(0:4cm) node {]} node[below=0.2cm] {$\LS[2]$}; \draw
(15,0) node {[} node[above=0.2cm] {$\EE[2]$}-- +(0:4cm) node {]}
node[right=0.1cm,above=0.2cm] {$\LE[2]$}; \draw(10.1,0) node {[}
node[below=0.3cm] {$\ES[3]$}-- +(0:3cm) node {]} node[below=0.2cm]
{$\LS[3]$}; \draw (18,0) node {[} node[below=0.2cm] {$\EE[3]$}--
+(0:4cm) node {]} node[below=0.25cm] {$\LE[3]$};
\end{tikzpicture}
\end{center}

Après avoir trier les intervalles, nous obtenons:
$[\ES[1],\LS[1]] \le [\ES[2],\LS[2]] \le
[\EE[1],\LE[1]] \le [\ES[3],\LS[3]]\le
[\EE[2],\LE[2]] \le [\EE[3],\LE[3]]$.

Alors, nous avons l'ensemble de contraintes suivants:

\begin{itemize}
\item $t_2-t_1 \le \LS[2]-\ES[1]$
\item $t_3-t_2 \le \LE[1]-\ES[2]$
\item $t_5-t_4 \le \LE[2]-r_3$
\item $t_6-t_5 \le d_3-\EE[2]$
\end{itemize}
\end{ex}
...
dans~\eqref{bmin_CECSP_OO}. L'inégalité se réécrirait donc comme:
